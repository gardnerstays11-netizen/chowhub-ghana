...
export default function AdminLoginScreen() {
  ...
  const handleLogin = () => {
    ...
    mutation.mutate(
      ...
      {
        onSuccess: async (res) => {
          await loginAdmin(res.token, res.user as any);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: isWeb ? 67 + 40 : 40 }]}>
      <Text style={[styles.title, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Admin Portal</Text>
      <Text style={[styles.subtitle, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Platform management</Text>

      <View style={styles.form}>
        <Text style={[styles.label, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>Email</Text>
        <TextInput value={email} onChangeText={setEmail} placeholder="admin@chowhub.gh" placeholderTextColor={colors.mutedForeground} keyboardType="email-address" autoCapitalize="none" style={[styles.input, { borderColor: colors.border, color: colors.foreground, borderRadius: colors.radius, fontFamily: "Inter_400Regular" }]} />
