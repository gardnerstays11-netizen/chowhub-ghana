...
export default function MyReservationsScreen() {
  ...
  const { data, isLoading } = useGetMyReservations({ query: { enabled: isAuthenticated } as any });
      ...
      <FlatList
        ...
        renderItem={({ item }: { item: any }) => (
          <View style={[styles.item, { borderColor: colors.border, backgroundColor: colors.card, borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.meta, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>{item.date} · {item.time} · {item.partySize} guests</Text>
          </View>
        )}
        ListEmptyComponent={
